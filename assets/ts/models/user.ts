...
export interface User {
    id: number;
    username: string;
    password?: string; // Optionnel car on ne veut pas toujours l'inclure dans les objets
    follows_user: number[];
}
...
export class UserModel {
     ...
     */
    async followUser(userId: number, targetUserId: number): Promise<User> {
        try {
     ...
     */
    async unfollowUser(userId: number, targetUserId: number): Promise<User> {
        try {
            // Récupérer l'utilisateur actuel
            const user = await this.getUserById(userId);
    ...
    async getFollowedUsers(userId: number): Promise<User[]> {
        try {
            ...
            const followedUsers = await Promise.all(
                user.follows_user.map(id => this.getUserById(id))
            );
    ...
    async getFollowers(userId: number): Promise<User[]> {
        try {
            ...
            const followers = allUsers.filter(user => 
                user.follows_user.includes(userId)
            );
