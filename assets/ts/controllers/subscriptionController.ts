...
export class SubscriptionController {
     ...
     */
    public async showSubscriptionsPage(): Promise<void> {
     ...
     */
    private addSubscriptionPageEventListeners(allUsers: User[]): void {
        const followButton = document.getElementById('follow-button');
        const searchUsernameInput = document.getElementById('search-username') as HTMLInputElement;
        ...
        unfollowButtons.forEach(button => {
            button.addEventListener('click', async () => {
                const targetUserId = parseInt((button as HTMLElement).dataset.userId || '0', 10);
                if (targetUserId) {
     ...
     */
    private async handleFollowUser(username: string, allUsers: User[]): Promise<void> {
     ...
     */
    private async handleUnfollowUser(targetUserId: number): Promise<void> {
        const userId = AuthService.getUserId();
