...
export class FeedController {
     ...
     */
    public async showFeedPage(): Promise<void> {
        const mainContainer = document.getElementById('main-container');
     ...
     */
    public async showPostsPage(): Promise<void> {
        const mainContainer = document.getElementById('main-container');
        if (!mainContainer) return;
        ...
        try {
            ...
            const ticketsForReviews = await Promise.all(
                userReviews.map(review => this.ticketModel.getTicketById(review.ticketId))
            );
     ...
     */
    private addFeedPageEventListeners(): void {
        ...
        const filterCheckbox = document.getElementById('filter-checkbox') as HTMLInputElement;
        ...
        createReviewForTicketBtns.forEach(button => {
            button.addEventListener('click', () => {
                const ticketId = parseInt((button as HTMLElement).dataset.ticketId || '0', 10);
                if (ticketId) {
     ...
     */
    private addPostsPageEventListeners(): void {
        ...
        editReviewBtns.forEach(button => {
            button.addEventListener('click', () => {
                const reviewId = parseInt((button as HTMLElement).dataset.reviewId || '0', 10);
                if (reviewId) {
