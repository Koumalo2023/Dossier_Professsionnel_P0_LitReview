...
export class TicketModel {
     ...
     */
    async getTicketsSortedByDate() {
        try {
            const tickets = await this.getAllTickets();
            return tickets.sort((a, b) => new Date(b.time_created).getTime() - new Date(a.time_created).getTime());
        }
        catch (error) {
     ...
     */
    async getTicketsForFeed(userId, followedUserIds) {
        try {
            // Récupérer tous les tickets
            const allTickets = await this.getAllTickets();
            // Filtrer les tickets pour n'inclure que ceux de l'utilisateur et des utilisateurs suivis
            const relevantTickets = allTickets.filter(ticket => ticket.userId === userId || followedUserIds.includes(ticket.userId));
            // Trier par date (du plus récent au plus ancien)
            return relevantTickets.sort((a, b) => new Date(b.time_created).getTime() - new Date(a.time_created).getTime());
        }
        catch (error) {
